...
function fetchPuestos(empresaID) {
    fetch(`http://localhost:4000/empresa/puestos/${empresaID}`)
...
function fetchSolicitudes(empresaID) {
    fetch(`http://localhost:4000/empresas/solicitudes/get/${empresaID}`)
        .then(response => {
            if (!response.ok) {
...
function renderDetaSol(id) {
    fetch(`http://localhost:4000/solicitudes/${id}`)
        .then(response => response.json())
        .then(data => {
            if (!data) {
...
function contratarSolicitante() {
    ...
    fetch('http://localhost:4000/empresa/contratar', {
        method: 'PUT',
        headers: {
